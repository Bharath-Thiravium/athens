...
class CanManagePermits(permissions.BasePermission):
    ...
    - CREATE: contractoruser (any grade), clientuser (C grade), epcuser (C grade)
    ...
    def has_permission(self, request, view):
        ...
        if request.method == 'POST':
            if (hasattr(request.user, 'django_user_type') and request.user.django_user_type == 'adminuser'):
                if hasattr(request.user, 'usertype') and request.user.usertype == 'contractoruser':
                    return True
                if (hasattr(request.user, 'usertype') and request.user.usertype in ['clientuser', 'epcuser'] and
                    hasattr(request.user, 'grade') and request.user.grade == 'C'):
                    return True
            return False
        ...
        if request.method in ['PUT', 'PATCH']:
            if (hasattr(request.user, 'django_user_type') and request.user.django_user_type == 'adminuser' and
                hasattr(request.user, 'usertype') and request.user.usertype in ['clientuser', 'epcuser'] and
...
class CanApprovePermits(permissions.BasePermission):
    ...
    - EPC B grade: approve EPC permits
    ...
    def has_permission(self, request, view):
        ...
        # Allow client users (C/B/A grade) and EPC B grade to approve
        if (hasattr(request.user, 'django_user_type') and request.user.django_user_type == 'adminuser'):
            if (hasattr(request.user, 'usertype') and request.user.usertype == 'clientuser' and
                hasattr(request.user, 'grade') and request.user.grade in ['A', 'B', 'C']):
                return True
            if (hasattr(request.user, 'usertype') and request.user.usertype == 'epcuser' and
                hasattr(request.user, 'grade') and request.user.grade == 'B'):
                return True
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if (hasattr(request.user, 'django_user_type') and request.user.django_user_type == 'adminuser' and
            hasattr(request.user, 'usertype') and request.user.usertype == 'clientuser' and
            hasattr(request.user, 'grade') and request.user.grade in ['A', 'B', 'C']):
            return True

        # EPC B grade can approve EPC permits
        if (hasattr(request.user, 'django_user_type') and request.user.django_user_type == 'adminuser' and
            hasattr(request.user, 'usertype') and request.user.usertype == 'epcuser' and
            hasattr(request.user, 'grade') and request.user.grade == 'B'):
            return True
