...
export default function PTWRecordPrintPreview({ permitData }: PTWRecordPrintPreviewProps) {
  ...
  const findSignatureByType = (permit: any, type: string) => {
    ...
    if (permit.signatures_by_type && permit.signatures_by_type[type]) {
      return permit.signatures_by_type[type];
    }
  ...
  const renderSignature = (type: string, permit: any, label: string) => {
    ...
    // For signed signatures, render the actual signature image
    ...
    return `
      <div class="signature-box">
        ...
        <div class="signature-image-container">
          ${signatureImageSrc ? `<img src="${signatureImageSrc}" alt="${label} Signature" class="signature-template-image" />` : `
            <div class="adobe-signature-block">
              <div class="signature-partitions">
                <div class="signature-left">
                  <div class="signer-name">Signature Missing</div>
                </div>
                <div class="signature-divider"></div>
                <div class="signature-right">
                  <div class="signed-by">Template Error</div>
                </div>
              </div>
            </div>
          `}
      ...
      <div class="document-container">
        ...
        <div class="section">
          ...
          <div class="signature-grid">
            ...
            ${renderSignature('issuer', permitData, 'Issuer')}
            ${renderSignature('receiver', permitData, 'Receiver')}
            ${renderSignature('safety_officer', permitData, 'Safety Officer')}
            ${renderSignature('area_manager', permitData, 'Area Manager')}
            ${renderSignature('witness', permitData, 'Witness')}
          </div>
        </div>
        ...
        <head>
          ...
          <style>
            ...
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .signature-image-container { flex: 1; display: flex; align-items: center; justify-content: center; min-height: 70px; }
            .signature-template-image { max-width: 100%; max-height: 70px; object-fit: contain; border: 1px solid #ddd; }
