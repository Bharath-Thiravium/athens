...
class UnifiedPTWWorkflowManager:
    ...
    def transition_status(self, permit, new_status, user, comments='', **kwargs):
        ...
        if not self._can_transition(permit, new_status, user):
            raise ValueError(f"Invalid transition from {permit.status} to {new_status} by {user}")
        
        old_status = permit.status
        permit.status = new_status
        permit.save()
        
        self._create_audit_log(permit, new_status, user, comments)
        self._send_transition_notifications(permit, old_status, new_status, user)
