...
export const usePermissions = (): UsePermissionsReturn => {
  ...
  const isMasterAdmin = user?.admin_type === 'master';
  const isProjectAdmin = user?.admin_type && ['client', 'epc', 'contractor'].includes(user.admin_type);
  const isAdminUser = user?.admin_type && ['clientuser', 'epcuser', 'contractoruser'].includes(user.admin_type);
  const userGrade = user?.grade || null;
  const adminType = user?.admin_type || null;
