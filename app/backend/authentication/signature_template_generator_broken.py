...
class SignatureTemplateGenerator:
    ...
    def create_signature_template(self, user_detail):
        ...
        # Layout - equal left and right columns
        right_margin = 225  # Half of 450px width
        ...
        draw.text((left_margin, left_y), full_name, font=name_font, fill=text_color)
        left_y += 30
        
        # Designation on left side
        designation = user_detail.user.designation or ""
        if designation:
            draw.text((left_margin, left_y), designation, font=detail_font, fill=text_color)
            left_y += line_height
        
        # Department on left side
        department = user_detail.user.department or ""
        if department:
            draw.text((left_margin, left_y), department, font=detail_font, fill=text_color)
        ...
        if company_name:
            draw.text((right_margin, right_y), company_name, font=detail_font, fill=text_color)
