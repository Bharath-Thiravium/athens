...
from .company_isolation import get_tenant_isolated_queryset, TenantIsolationMixin
...
class MasterAdminCreateProjectAdminsView(APIView):
    def post(self, request):
        ...
        created_admins = []
        existing_admins = []
        project_tenant_id = project.athens_tenant_id
        ...
        if client_username:
            if CustomUser.objects.filter(username=client_username).exists():
                ...
                client_data = {
                    ...
                    'is_active': True,
                    'athens_tenant_id': project_tenant_id,
                }
...
class ProjectListView(APIView):
    ...
    def get(self, request):
        ...
        if user.user_type in ['master', 'masteradmin'] or user.admin_type in ['master', 'masteradmin']:
            # Master users see all projects with their tenant ID
            ...
            # If user has no project assigned, return empty list
...
class ProjectAdminUserListView(APIView):
    ...
    COMPANY-BOUNDED: Only shows users from the same company/project.
    """
    permission_classes = (IsAuthenticated,)

    def get(self, request):
        # COMPANY ISOLATION: Filter users by same project only
        if not request.user.project:
            return Response([], status=status.HTTP_200_OK)
            
        users = CustomUser.objects.filter(
            user_type='adminuser', 
            project=request.user.project  # Same company/project only
...
class CurrentUserProjectView(APIView):
    ...
    def get(self, request):
        ...
        return Response({
            'project': project_info,
            'user_type': normalize_master_type(user.user_type),
            'admin_type': normalize_master_type(getattr(user, 'admin_type', None)),
        }, status=status.HTTP_200_OK)
            ...
            try:
                ...
                response_data = {
                    'admin_detail': {
                        ...
                        'phoneNumber': admin_detail_data.get('phone_number', ''),
                        'panNumber': admin_detail_data.get('pan_number', ''),
                        'gstNumber': admin_detail_data.get('gst_number', ''),
                        'phone': admin_detail_data.get('phone_number', ''),
                        'pan': admin_detail_data.get('pan_number', ''),
                        'gst': admin_detail_data.get('gst_number', ''),
                        'companyName': user.company_name,
                        'registeredAddress': user.registered_address,
                        'company': user.company_name,
                        'address': user.registered_address,
                        'profilePhoto': admin_detail_data.get('photo'),
                        'companyLogo': admin_detail_data.get('logo'),
                        'profile_photo': admin_detail_data.get('photo'),
                        'company_logo': admin_detail_data.get('logo'),
                        'image': admin_detail_data.get('photo'),
                        'logo_image': admin_detail_data.get('logo'),
                        'photo_url': admin_detail_data.get('photo'),
                        'logo_url': admin_detail_data.get('logo')
                    }
                }
    ...
    def get(self, request):
        try:
            ...
            response_data = {
                ...
                'user_type': normalize_master_type(user.user_type),
                'admin_type': normalize_master_type(user.admin_type),
                'debug_info': {
                    'user_id': user.id,
                    'username': user.username,
...
class CurrentUserProfileView(APIView):
    """
    Get current user's basic profile data from CustomUser model
    """
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        ...
        try:
            if user.user_type == 'adminuser':
                # Try to get from UserDetail
                try:
                    user_detail = UserDetail.objects.get(user=user)
                    if user_detail.photo:
                        profile_pic_url = request.build_absolute_uri(user_detail.photo.url)
                except UserDetail.DoesNotExist:
                    pass
            elif user.user_type == 'projectadmin':
                # Try to get from AdminDetail
                try:
                    admin_detail = AdminDetail.objects.get(user=user)
                    if admin_detail.photo:
                        profile_pic_url = request.build_absolute_uri(admin_detail.photo.url)
                except AdminDetail.DoesNotExist:
                    pass
        except Exception as e:
            logger.debug(f"Error getting profile picture: {sanitize_log_input(str(e))}")
        
        # Get employee ID from UserDetail if available
        employee_id = 'N/A'
        try:
            if user.user_type == 'adminuser':
                user_detail = UserDetail.objects.get(user=user)
                employee_id = user_detail.employee_id or 'N/A'
        except UserDetail.DoesNotExist:
            pass
        except Exception as e:
            logger.debug(f"Error getting employee ID: {sanitize_log_input(str(e))}")
        ...
        response_data = {
            ...
            'project_name': user.project.projectName if user.project else None
        }
    ...
    def get(self, request):
        ...
        try:
            if user.athens_tenant_id:
                ...
                from control_plane.models import TenantCompany, SaaSSubscription
                
                try:
                    tenant_company = TenantCompany.objects.get(id=user.athens_tenant_id)
                    subscription = SaaSSubscription.objects.filter(tenant=tenant_company).first()
                    
                    if subscription:
                        subscription_status = subscription.status
                        is_trialing = subscription.status == 'trialing'
                    else:
                        # No subscription record means default trial
                        is_trialing = True
                        subscription_status = 'trialing'
                        
                except TenantCompany.DoesNotExist:
                    # Tenant not found, assume trial
                    is_trialing = True
                    subscription_status = 'trialing'
            else:
                # No tenant ID, assume trial for safety
                is_trialing = True
                subscription_status = 'trialing'
                
        except Exception as e:
            logger.debug(f"Error getting subscription status: {sanitize_log_input(str(e))}")
        ...
        return Response({
            ...
            'tenant_id': str(user.athens_tenant_id) if user.athens_tenant_id else None
        }, status=status.HTTP_200_OK)

class MenuManagementStatsView(APIView):
    """
    Consolidated KPI and statistics for Menu Management module
    """
    permission_classes = [IsAuthenticated]

    def get(self, request):
        if request.user.admin_type not in ['master', 'masteradmin']:
            return Response({'error': 'Only master admin can view menu management stats'}, status=status.HTTP_403_FORBIDDEN)

        try:
            user_tenant_id = getattr(request.user, 'athens_tenant_id', None)
            # Get all projects for the tenant
            projects = Project.objects.filter(athens_tenant_id=user_tenant_id) if user_tenant_id else Project.objects.all()
            total_projects = projects.count()
            
            # Menu modules stats
            try:
                from .menu_models import MenuModule, ProjectMenuAccess
                total_modules = MenuModule.objects.filter(is_active=True).count()
                access_qs = ProjectMenuAccess.objects.all()
                if user_tenant_id:
                    access_qs = access_qs.filter(project__athens_tenant_id=user_tenant_id)
                total_menu_configs = access_qs.count()
                enabled_configs = access_qs.filter(is_enabled=True).count()
            except:
                total_modules = 8  # Fallback count
                total_menu_configs = 0
                enabled_configs = 0
            
            # Project-wise menu access stats
            project_stats = []
            for project in projects:
                try:
                    project_configs = ProjectMenuAccess.objects.filter(project=project).count()
                    enabled_count = ProjectMenuAccess.objects.filter(project=project, is_enabled=True).count()
                except:
                    project_configs = 0
                    enabled_count = 0
                ...
                project_stats.append({
                    'project_id': project.id,
                    'project_name': project.projectName,
                    'total_configs': project_configs,
                    'enabled_configs': enabled_count,
                    'coverage_percentage': round((enabled_count / total_modules * 100) if total_modules > 0 else 0, 1)
                })
            ...
            kpis = [
                ...
                },
                {
                    ...
                    'value': total_menu_configs,
                    'icon': 'setting',
                    'color': '#faad14',
                    'trend': 'up' if total_menu_configs > 0 else 'stable'
                },
                {
                    'title': 'Active Configurations',
                    'value': enabled_configs,
                    'icon': 'check-circle',
                    'color': '#13c2c2',
                    'trend': 'up' if enabled_configs > 0 else 'stable'
                }
            ]
            ...
            stats = {
                ...
                'total_configurations': total_menu_configs,
                'enabled_configurations': enabled_configs,
                'configuration_rate': round((total_menu_configs / (total_projects * total_modules) * 100) if total_projects > 0 and total_modules > 0 else 0, 1),
                'activation_rate': round((enabled_configs / total_menu_configs * 100) if total_menu_configs > 0 else 0, 1)
            }
            
            # Chart data
            charts = {
                'project_coverage': project_stats[:10],  # Top 10 projects
                'module_distribution': [
                    {'name': 'Dashboard', 'value': 1, 'color': '#1890ff'},
                    {'name': 'Workers', 'value': 1, 'color': '#52c41a'},
                    {'name': 'PTW', 'value': 1, 'color': '#faad14'},
                    {'name': 'Incidents', 'value': 1, 'color': '#f5222d'},
                    {'name': 'Inspections', 'value': 1, 'color': '#722ed1'},
                    {'name': 'Training', 'value': 1, 'color': '#13c2c2'},
                    {'name': 'Manpower', 'value': 1, 'color': '#eb2f96'},
                    {'name': 'Safety Observation', 'value': 1, 'color': '#fa8c16'}
                ],
                'configuration_status': [
                    {'name': 'Configured', 'value': enabled_configs, 'color': '#52c41a'},
                    {'name': 'Not Configured', 'value': total_menu_configs - enabled_configs, 'color': '#f5222d'}
                ]
            }
            ...
            logger.error(f"Error in MenuManagementStatsView: {sanitize_log_input(str(e))}")
            return Response({
                'success': False,
                'error': 'Failed to fetch menu management statistics'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    """
    Consolidated dashboard data for admin users module before project selection
    """
    permission_classes = [IsAuthenticated]

    def get(self, request):
        # Only master admin can view consolidated data
        if request.user.admin_type not in ['master', 'masteradmin']:
            return Response({'error': 'Only master admin can view consolidated data'}, status=status.HTTP_403_FORBIDDEN)
        ...
        try:
            ...
            for project in projects:
                # Get project admins
                project_admins = CustomUser.objects.filter(
                    project=project,
                    user_type='projectadmin',
                    is_active=True
                )
                ...
                # Count by admin type
                project_client_admins = project_admins.filter(admin_type='client').count()
                project_epc_admins = project_admins.filter(admin_type='epc').count()
                project_contractor_admins = project_admins.filter(admin_type='contractor').count()
                
                project_client_users = project_users.filter(admin_type='clientuser').count()
                project_epc_users = project_users.filter(admin_type='epcuser').count()
                project_contractor_users = project_users.filter(admin_type='contractoruser').count()
                
                # Get pending approvals for this project
                project_pending = 0
                for admin in project_admins:
                    try:
                        admin_detail = AdminDetail.objects.get(user=admin)
                        if not admin_detail.is_approved:
                            project_pending += 1
                    except AdminDetail.DoesNotExist:
                        pass
                ...
                for user in project_users:
                    try:
                        user_detail = UserDetail.objects.get(user=user)
                        if not user_detail.is_approved:
                            project_pending += 1
                    except UserDetail.DoesNotExist:
                        pass
                
                project_total_admins = project_admins.count()
                project_total_users = project_users.count()
                
                # Add to totals
                total_admins += project_total_admins
                total_users += project_total_users
                pending_approvals += project_pending
                
                client_admins += project_client_admins
                epc_admins += project_epc_admins
                contractor_admins += project_contractor_admins
                
                client_users += project_client_users
                epc_users += project_epc_users
                contractor_users += project_contractor_users
                
                if project_total_admins > 0 or project_total_users > 0:
                    active_projects += 1
            ...
            kpis = [
                ...
                },
                {
                    ...
                    'target': projects.count(),
                    'unit': '',
                    'trend': 'up' if active_projects > 0 else 'stable',
                    'icon': 'active'
                },
                {
                    'name': 'Total Admins',
                    ...
                    'target': total_admins,
                    'unit': '',
                    'trend': 'up' if total_admins > 0 else 'stable',
                    'icon': 'admin'
                },
                {
                    ...
                    'target': 0,
                    'unit': '',
                    'trend': 'down' if pending_approvals == 0 else 'warning',
                    'icon': 'pending'
                }
            ]
            ...
            statistics = {
                ...
                'admin_breakdown': {
                    ...
                    'epc_admins': epc_admins,
                    'contractor_admins': contractor_admins
                },
                'user_breakdown': {
            ...
            # Chart data
            charts = {
                'project_distribution': project_data[:10],  # Top 10 projects
                'admin_type_distribution': [
                    {'name': 'Client Admins', 'value': client_admins, 'color': '#22c55e'},
                    {'name': 'EPC Admins', 'value': epc_admins, 'color': '#3b82f6'},
                    {'name': 'Contractor Admins', 'value': contractor_admins, 'color': '#f59e0b'}
                ],
                'user_type_distribution': [
                    {'name': 'Client Users', 'value': client_users, 'color': '#22c55e'},
                    {'name': 'EPC Users', 'value': epc_users, 'color': '#3b82f6'},
                    {'name': 'Contractor Users', 'value': contractor_users, 'color': '#f59e0b'}
                ],
                'approval_status': [
                    {'name': 'Approved', 'value': (total_admins + total_users) - pending_approvals, 'color': '#22c55e'},
                    {'name': 'Pending', 'value': pending_approvals, 'color': '#f59e0b'}
                ]
            }
            ...
            logger.error(f"Error in ConsolidatedAdminDashboardView: {sanitize_log_input(str(e))}")
            return Response({
                'success': False,
                'error': 'Failed to fetch consolidated dashboard data'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
