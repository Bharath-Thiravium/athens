...
class Project(models.Model):
    """
    CORRECT: Business Project Model
    
    A Project represents a business project that:
    - Belongs to one company (athens_tenant_id)
    - Can involve multiple companies (client, EPC, contractors)
    - Is NOT used for company isolation
    """
    ...
    client_company_id = models.UUIDField(
        null=True, blank=True,
        help_text="Client company athens_tenant_id"
    )
    epc_company_id = models.UUIDField(
        null=True, blank=True,
        help_text="EPC company athens_tenant_id"
    )
    contractor_company_ids = models.JSONField(
        default=list,
        help_text="List of contractor company athens_tenant_ids"
    )
...
class CustomUserManager(BaseUserManager):
    ...
    def create_user(self, username, password=None, email=None, user_type=None, **extra_fields):
        ...
        # Skip project binding for platform superadmin accounts.
        skip_project_binding = user_type == 'superadmin'
        if not skip_project_binding and 'project' not in extra_fields and 'project_id' not in extra_fields:
