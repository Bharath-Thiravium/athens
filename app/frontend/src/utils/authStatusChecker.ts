  ...
  static async refreshAuth(): Promise<boolean> {
    try {
      ...
      if (response.data.access) {
        const tokenInfo = useAuthStore.getState();
        ...
        authStore.setToken(
          ...
          tokenInfo.projectId,
          tokenInfo.username,
          decoded?.usertype || tokenInfo.usertype,
          decoded?.django_user_type || tokenInfo.django_user_type,
          decoded?.isSuperAdmin || false,
          tokenInfo.userId,
          tokenInfo.isPasswordResetRequired,
          tokenInfo.grade,
          tokenInfo.department,
          tokenInfo.isApproved,
          tokenInfo.hasSubmittedDetails
        );
