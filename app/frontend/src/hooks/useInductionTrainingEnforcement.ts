...
export const useInductionTrainingEnforcement = () => {
  ...
  const isSuperAdmin = (): boolean => usertype === 'superadmin';
  ...
  useEffect(() => {
    const enforceInductionTraining = async () => {
      ...
      // Skip for platform/tenant admins and EPC Safety users
      if (isSuperAdmin() || isMasterAdmin() || isProjectAdmin() || isEPCSafetyUser()) {
        return;
      }
