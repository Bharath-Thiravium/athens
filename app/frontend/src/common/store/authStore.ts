...
const getNormalizedStoredItem = (key: string): string | null => {
  const value = getStoredItem(key);
  const normalizedValue = normalizeUserType(value);
  if (typeof window !== 'undefined' && value && normalizedValue && value !== normalizedValue) {
...
const useAuthStore = create<AuthState>((set, get) => ({
  ...
  usertype: getNormalizedStoredItem('usertype'),
  django_user_type: getNormalizedStoredItem('django_user_type'),
  isSuperAdmin: getStoredItem('isSuperAdmin') === 'true',
  ...
  setToken: (
    ...
    const normalizedDjangoUserType = normalizeUserType(django_user_type);
    ...
    if (typeof window !== 'undefined') {
      ...
      if (normalizedDjangoUserType) {
        localStorage.setItem('django_user_type', normalizedDjangoUserType);
      } else {
    ...
    set({
      ...
      django_user_type: normalizedDjangoUserType,
      isSuperAdmin,
      userId,
  ...
  setUsertype: (usertype: string | null) => {
    ...
    set({ usertype: normalizedUsertype });
  },
  setDjangoUserType: (django_user_type: string | null) => {
    ...
    set({ django_user_type: normalizedDjangoUserType });
  },
