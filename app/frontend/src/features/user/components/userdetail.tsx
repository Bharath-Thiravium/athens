...
const UserDetail: React.FC<UserDetailProps> = ({ initialMobile }) => {
  ...
  const fetchUserDetail = useCallback(async () => {
    ...
    try {
      ...
      if (userToApprove?.user) {
        ...
        // If name/surname are missing from UserDetail, try to get from auth store
        if (!data.name || !data.surname) {
          const authStore = useAuthStore.getState();
          data.name = data.name || authStore.username || '';
  ...
  const onFinish = useCallback(async (values: any) => {
    ...
    formPayload.append('surname', values.surname || '');
    formPayload.append('designation', values.designation || '');
    formPayload.append('department', values.department || '');
  ...
  return (
    <UserDetailContainer>
      <Form form={form} layout="vertical" onFinish={onFinish}>
        ...
        <Row gutter={32}>
          ...
          <Col xs={24} md={16}>
            <Section>
                ...
                <Row gutter={24}>
                    <Col xs={24} sm={12}><Form.Item label="Name" name="name" rules={[{ required: true, message: 'Name is required' }]}><Input disabled={isReadOnly} /></Form.Item></Col>
                    <Col xs={24} sm={12}><Form.Item label="Surname" name="surname" rules={[{ required: true, message: 'Surname is required' }]}><Input disabled={isReadOnly} /></Form.Item></Col>
          ...
          <Col xs={24} md={8}>
            <div className="profile-photo-card" style={{
              ...
              <SectionTitle level={4} style={{ textAlign: 'center', marginBottom: '20px' }}>Profile Photo</SectionTitle>
              <Form.Item 
                name="photo" 
                rules={[{ required: !initialPhotoUrl && !fileObjects.photo, message: 'Profile photo is required' }]}
        ...
        <Section>
            <SectionTitle level={4}><FileTextOutlined />Document Attachments</SectionTitle>
            <Row gutter={[24, 16]}>
                <Col xs={24} lg={12}>
                    <div className="attachment-card" style={{
                        ...
                        <div style={{ marginBottom: '16px' }}>
                            <Text strong style={{ color: 'var(--color-text-base)', fontSize: '14px' }}>PAN Card</Text>
                            <Text type="secondary" style={{ display: 'block', fontSize: '12px', marginTop: '4px' }}>Upload clear image or PDF of PAN card</Text>
                        </div>
                        <Form.Item 
                            name="panAttachment" 
                ...
                <Col xs={24} lg={12}>
                    <div className="attachment-card" style={{
                        ...
                        <div style={{ marginBottom: '16px' }}>
                            <Text strong style={{ color: 'var(--color-text-base)', fontSize: '14px' }}>Aadhaar Card</Text>
                            <Text type="secondary" style={{ display: 'block', fontSize: '12px', marginTop: '4px' }}>Upload clear image or PDF of Aadhaar card</Text>
                        </div>
                        <Form.Item 
                ...
                <Col xs={24}>
                    <div className="signature-card" style={{
                        ...
                        <div style={{ marginBottom: '16px' }}>
                            <Text strong style={{ color: 'var(--color-text-base)', fontSize: '14px' }}>Digital Signature</Text>
                            <Text type="secondary" style={{ display: 'block', fontSize: '12px', marginTop: '4px' }}>Create your digital signature template</Text>
                        </div>
                        <Form.Item name="signatureTemplate" style={{ marginBottom: 0 }}>
        ...
        {!isApproved || userToApprove ? (
            <div style={{
                ...
                <Form.Item style={{ marginBottom: 0 }}>
                    ...
                    >
                        {userToApprove ? 'Update & Approve User' : 'Submit for Approval'}
                    </Button>
                    {!userToApprove && (
                        <div style={{ marginTop: '12px' }}>
                            <Text type="secondary" style={{ fontSize: '12px' }}>
                                Your details will be reviewed by an administrator
                            </Text>
                        </div>
                    )}
                </Form.Item>
