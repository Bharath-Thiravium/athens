...
import { 
  ...
  safeMap
} from '../../../utils/defensiveUtils';
import { DashboardCardErrorBoundary, ChartErrorBoundary } from '@common/components/ErrorBoundary';
...
const DashboardOverview: React.FC = () => {
  ...
  const [dataReady, setDataReady] = useState(true); // Start with mock data ready
  ...
  // Handle time range change with safe data validation
  const handleTimeRangeChange = (value: string) => {
    setTimeRange(value);
    
    try {
      // Validate and set mock data safely
      ...
      const validation = validateComponentState.dashboard(updatedData);
      setDashboardData(validation.data);
      setDataReady(validation.isValid);
    } catch (error) {
  ...
  // CRITICAL: Block rendering if data is not ready or error occurred
  if (error || !dataReady) {
    return (
      <div className="space-y-6" style={{ paddingTop: 80 }}>
        <Alert
          message="Dashboard Unavailable"
          description="Dashboard data is currently unavailable. Please refresh the page or try again later."
          type="warning"
          showIcon
          action={
  ...
  // CRITICAL: Prepare chart data with mandatory sanitization
  const safetyTrendData = chartSafe.data(
    dashboardData?.charts?.safety_trend || []
  );

  const permitStatusData = chartSafe.pieData(
    dashboardData?.charts?.permit_status || []
  );

  return (
    <PageLayout
      title="Dashboard Overview"
      subtitle="Real-time insights and key performance indicators"
    ...
    >
      ...
      {/* KPI Cards Row - CRITICAL: Wrapped in error boundary */}
      <Row gutter={[16, 16]} className="mb-6">
        {safeArray(dashboardData?.kpis || []).map((kpi: any, index: number) => (
          <Col xs={24} sm={12} lg={6} key={index}>
            <DashboardCardErrorBoundary title={`KPI Card ${index + 1}`}>
              <Card className="text-center">
                <Statistic
                  {...antdSafe.statistic(kpi?.value, kpi?.unit)}
                  title={safeString(kpi?.name, 'N/A')}
                  valueStyle={{ 
                    color: safeString(kpi?.trend) === 'up' ? '#3f8600' : 
                           safeString(kpi?.trend) === 'down' ? '#cf1322' : '#1890ff' 
                  }}
                  prefix={
                    safeString(kpi?.trend) === 'up' ? <ArrowUpOutlined /> : 
                    safeString(kpi?.trend) === 'down' ? <ArrowDownOutlined /> : <RiseOutlined />
                  }
                />
                <Text className="text-xs text-color-text-muted">
                  Target: {safeNumber(kpi?.target, 0)}{safeString(kpi?.unit, '')}
                </Text>
              </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <Title level={2} className="!text-color-text-base !mt-2 !mb-0">
                        {safeNumber(dashboardData?.statistics?.permits?.total, 0).toLocaleString()}
                      </Title>
                      <Text className="text-xs text-color-text-muted">
                        {safeNumber(dashboardData?.statistics?.permits?.this_period, 0)} new this {timeRange}
                      </Text>
                    </div>
                    <div className="text-right">
                      <FileTextOutlined className="text-2xl text-blue-500 mb-2" />
                      <Tag
                        color={safeNumber(dashboardData?.statistics?.permits?.change_percentage, 0) >= 0 ? "success" : "error"}
                        icon={safeNumber(dashboardData?.statistics?.permits?.change_percentage, 0) >= 0 ? <ArrowUpOutlined /> : <ArrowDownOutlined />}
                        className="font-semibold"
                      >
                        {Math.abs(safeNumber(dashboardData?.statistics?.permits?.change_percentage, 0))}%
                      </Tag>
                    </div>
                  </div>
              </Card>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Safety Trend Chart - CRITICAL: Wrapped in error boundary */}
            <DashboardCardErrorBoundary title="Safety Trend Chart">
