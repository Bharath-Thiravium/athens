 ...
 */
const isEPCSafetyUser = (usertype?: string | null, department?: string | null): boolean => {
  const safeUsertype = usertype || '';
  const safeDepartment = department || '';
  return safeUsertype === 'epcuser' && safeDepartment && safeDepartment.toLowerCase().includes('safety');
};

/**
 * Get training menu items based on user permissions
 */
const getTrainingMenuItems = (usertype?: string | null, django_user_type?: string | null, department?: string | null): MenuItem[] => {
  const items: MenuItem[] = [];
  
  // Ensure safe string handling
  const safeUsertype = usertype || '';
  const safeDjangoUserType = django_user_type || '';
  const safeDepartment = department || '';

  items.push({ key: '/dashboard/training/check-in', icon: <QrcodeOutlined />, label: 'Training Check-in' });
  
  // EPC Safety Department gets induction training in addition to other training
  if (isEPCSafetyUser(safeUsertype, safeDepartment)) {
  ...
  if (safeDjangoUserType === 'adminuser') {
...
export const getMenuItemsForUser = (
  ...
  // If we have no user type information, provide a basic fallback menu
  if (!safeUsertype && !safeDjangoUserType) {
    console.warn('No user type information available, returning basic menu');
    return [
      { key: '/dashboard', icon: <DashboardOutlined />, label: 'Overview' },
  ...
  if (safeDjangoUserType === 'adminuser') {
    return [
      ...
      { key: '/dashboard/userdetail', icon: <UserOutlined />, label: 'User Detail' },
      {
        ...
        icon: <AuditOutlined />,
        label: 'Training',
        children: getTrainingMenuItems(safeUsertype, safeDjangoUserType, safeDepartment)
      },
      {
  ...
  if (['client', 'epc'].includes(safeUsertype ?? '') || safeUsertype?.includes('contractor')) {
