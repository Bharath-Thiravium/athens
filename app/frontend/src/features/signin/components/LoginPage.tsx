...
import { extractUserInfoFromToken } from '@common/utils/jwtUtils';
...
const LoginPage: React.FC = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      const { access, refresh, username, user_type, admin_type, userId, user_id, isPasswordResetRequired, grade, department, project_id, usertype, django_user_type } = response.data;
      const token = access || response.data.token;
      const decoded = token ? extractUserInfoFromToken(token) : null;
      const isSuperAdmin = decoded?.isSuperAdmin || user_type === 'superadmin';
      ...
      setToken(
        ...
        project_id ?? null,
        username,
        normalizedUserType, // Normalize legacy "master" to "masteradmin"
        normalizedAdminType, // Normalize legacy "master" to "masteradmin"
        isSuperAdmin,
        userId || user_id,
      ...
      // Debug log to verify data is set correctly
      console.log('Login successful - Auth data set:', {
        token: !!token,
        usertype: normalizedUserType,
        django_user_type: normalizedAdminType,
        username,
