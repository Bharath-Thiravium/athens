...
import { EditOutlined, DeleteOutlined, EyeOutlined, PlusOutlined, ProjectOutlined } from '@ant-design/icons';
...
const ProjectsList: React.FC = () => {
  ...
  const fetchProjects = useCallback(async () => {
    ...
    try {
      ...
      const normalizeProject = (project: any): Project => ({
        ...
        projectCategory: project.projectCategory ?? project.category ?? '',
        key: String(project.id),
      });
      const fetchedProjects: Project[] = Array.isArray(response.data)
        ? response.data.filter((p: any) => p.id != null).map(normalizeProject)
        : [];
  ...
  const columns = [
    { title: 'Project Name', dataIndex: 'projectName', key: 'projectName', render: (text: string) => <Text strong className="text-text-base">{text}</Text> },
    { title: 'Category', dataIndex: 'projectCategory', key: 'projectCategory', render: (text: string) => <Tag>{text ? text.charAt(0).toUpperCase() + text.slice(1).replace(/_/g, ' ') : 'Unknown'}</Tag> },
