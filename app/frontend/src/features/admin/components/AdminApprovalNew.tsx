...
import { Table, Button, Space, Card, Typography, Modal, Form, Input, Upload, message, Image, Tag, Descriptions, Select, Tabs, Row, Col, Divider, Statistic, Progress } from 'antd';
import { EyeOutlined, EditOutlined, CheckOutlined, CloseOutlined, UploadOutlined, UserOutlined, BankOutlined, PhoneOutlined, PlusOutlined, UserAddOutlined, KeyOutlined, DeleteOutlined, BuildOutlined, SyncOutlined, LockOutlined, ArrowUpOutlined, ArrowDownOutlined, TeamOutlined, ProjectOutlined, ExclamationCircleOutlined } from '@ant-design/icons';
...
const AdminApproval: React.FC = () => {
  ...
  useEffect(() => {
    if (!isAuthenticated() || userType !== 'masteradmin') {
  ...
  const fetchConsolidatedData = async () => {
    ...
    try {
      ...
      if (error.response?.status === 403) {
        message.error('Access denied. Only Master Admin can view consolidated data.');
      } else {
        message.error('Failed to fetch consolidated dashboard data');
      }
    } finally {
  ...
  return (
    <PageLayout
      ...
      subtitle="Create, manage and approve admin users across projects"
      icon={<UserOutlined />}
      breadcrumbs={[
        { title: 'Admin' },
        { title: 'Admin Users' }
    ...
    >
      ...
      {/* Consolidated Dashboard - Show when no project is selected */}
      {!selectedProjectId && consolidatedData && (
        <div className="space-y-6 mb-6">
          <Tabs defaultActiveKey="overview" type="card">
            <TabPane tab="Overview" key="overview">
              <Card>
                <Title level={4} className="!mb-4">System Overview</Title>
                
                {/* KPI Cards - High-level metrics */}
                <Row gutter={[16, 16]} className="mb-6">
                  {consolidatedData.kpis?.map((kpi: any, index: number) => (
                    <Col xs={24} sm={12} lg={6} key={index}>
                      <Card className="text-center">
                        <Statistic
                          ...
                          suffix={kpi.unit}
                          valueStyle={{ 
                            color: kpi.trend === 'up' ? '#3f8600' : 
                                   kpi.trend === 'down' ? '#cf1322' : 
                                   kpi.trend === 'warning' ? '#fa8c16' : '#1890ff' 
                          }}
                          prefix={
                            kpi.trend === 'up' ? <ArrowUpOutlined /> : 
                            kpi.trend === 'down' ? <ArrowDownOutlined /> : 
                            kpi.icon === 'project' ? <ProjectOutlined /> :
                            kpi.icon === 'admin' ? <UserOutlined /> :
                            kpi.icon === 'pending' ? <ExclamationCircleOutlined /> : null
                          }
                        />
                      </Card>
                    </Col>
                ...
                {/* Summary Cards - Current status */}
                <Row gutter={[16, 16]} className="mb-6">
                  <Col xs={24} sm={12} lg={8}>
                    <Card>
                      <div className="flex items-center justify-between">
                        <div>
                          <Text className="text-gray-500">Total Admins</Text>
                          <Title level={2} className="!mt-2 !mb-0">
                            {consolidatedData.statistics?.total_admins || 0}
                          </Title>
                    ...
                    <Card>
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <Title level={2} className="!mt-2 !mb-0">
                            {consolidatedData.statistics?.total_users || 0}
                          </Title>
                          <Text className="text-xs text-gray-500">
                            Admin users created
                          </Text>
                    ...
                    <Card>
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <Title level={2} className="!mt-2 !mb-0">
                            {consolidatedData.statistics?.pending_approvals || 0}
                          </Title>
                          <Text className="text-xs text-gray-500">
                            Require attention
                          </Text>
                ...
                <Row gutter={[16, 16]}>
                  <Col xs={24} lg={12}>
                    <Card>
                      <Title level={5} className="!mb-4">Approval Status</Title>
                      <div style={{ height: 200 }}>
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              data={consolidatedData.charts?.approval_status || []}
                              cx="50%"
                              cy="50%"
                              innerRadius={50}
                              outerRadius={80}
                              ...
                              paddingAngle={5}
                            >
                              {(consolidatedData.charts?.approval_status || []).map((entry: any, index: number) => (
                    ...
                    <Card>
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between p-3 bg-blue-50 rounded">
                          <span className="text-sm">New Admin Created</span>
                          <Tag color="blue">Today</Tag>
                        </div>
                        <div className="flex items-center justify-between p-3 bg-orange-50 rounded">
                          <span className="text-sm">Pending Approvals</span>
                          <Tag color="orange">{consolidatedData.statistics?.pending_approvals}</Tag>
                        </div>
                        <div className="flex items-center justify-between p-3 bg-green-50 rounded">
                          <span className="text-sm">Active Projects</span>
                          <Tag color="green">{consolidatedData.statistics?.active_projects}</Tag>
                        </div>
                      </div>
                    </Card>
                  </Col>
            ...
            <TabPane tab="Analytics" key="analytics">
              <Card>
                <Title level={4} className="!mb-4">Detailed Analytics</Title>
      ...
      {!selectedProjectId && (
        <Card>
          <div className="text-center py-8">
            <Text type="secondary">
              {consolidatedLoading ? 'Loading dashboard data...' : 'Please select a project to manage admin users'}
            </Text>
          </div>
        </Card>
      )}
        ...
        >
          ...
          <Form.Item label="Profile Photo" name="photo">
            <Upload
              ...
              onChange={(info) => {
                if (info.file) {
                  form.setFieldsValue({ photo: info.file });
                }
              }}
            >
              <div>
          ...
          <Form.Item label="Company Logo" name="logo">
            <Upload
              ...
              onChange={(info) => {
                if (info.file) {
                  form.setFieldsValue({ logo: info.file });
                }
              }}
            >
              <div>
