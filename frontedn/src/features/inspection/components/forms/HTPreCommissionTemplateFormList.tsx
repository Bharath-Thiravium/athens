...
const HTPreCommissionTemplateFormList: React.FC = () => {
      ...
      render: (_, record: any) => (
        <Space>
          <Button
            type="text"
            icon={<EyeOutlined />}
            onClick={() => navigate(`/dashboard/inspection/forms/ht-precommission-template/view/${record.id}`)}
          />
          {canEditDelete(record) && (
            <Button
              type="text"
              icon={<EditOutlined />}
              onClick={() => navigate(`/dashboard/inspection/forms/ht-precommission-template/edit/${record.id}`)}
            />
          )}
          {canEditDelete(record) && (
            <Button
  ...
  return (
    <PageLayout
      title="HT Pre-Commission Template Forms"
      subtitle="HT Cable Pre-Commissioning Checklist Template"
      breadcrumbs={[
      ...
      actions={canCreateForm ? [
        <Button
          ...
          type="primary"
          icon={<PlusOutlined />}
          onClick={() => navigate('/dashboard/inspection/forms/ht-precommission-template/create')}
        >
          Create New Form
        </Button>
      ] : []}
...
export default HTPreCommissionTemplateFormList;