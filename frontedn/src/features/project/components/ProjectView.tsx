...
const ProjectView: React.FC<ProjectViewProps> = ({ project, visible, onClose }) => {
  ...
  const getCategoryDisplay = (category: string) => {
    ...
    const label = categoryInfo.label || category.charAt(0).toUpperCase() + category.slice(1).replace(/_/g, ' ');
        ...
        <Row gutter={[32, 28]}>
            ...
            <Col span={24}>
                <DetailItem label="Project Name">
                    <Title level={4} className="!mt-0 !mb-0 !text-text-base">{project.name}</Title>
                </DetailItem>
            </Col>
